...
test.describe('Phase 14: Time + Project Analytics', () => {
  ...
  test('SC-2: velocity chart shows 4 bars on HealthCard', async ({ page }) => {
    await page.goto('/')
    await expect(page.locator('[data-testid="velocity-chart"]').first()).toBeVisible()
  ...
  test('SC-2: action trend indicator shows directional arrow', async ({ page }) => {
    await page.goto('/')
    const trendEl = page.locator('[data-testid="action-trend"]').first()
  ...
  test('SC-3: risk trend indicator visible on HealthCard', async ({ page }) => {
    await page.goto('/')
    const riskEl = page.locator('[data-testid="risk-trend"]').first()
    await expect(riskEl).toBeVisible()
    await expect(riskEl).toHaveText(/\d+ open risks?\s*[↑↓→]/)
  })
