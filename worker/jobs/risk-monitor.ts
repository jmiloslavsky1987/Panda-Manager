...
export default async function riskMonitorJob(job: Job): Promise<{ status: string }> {
  // 1. Acquire transaction-scoped advisory lock (auto-releases at transaction end)
  const [row] = await db.execute(
    sql`SELECT pg_try_advisory_xact_lock(${LOCK_IDS.RISK_MONITOR}) AS acquired`
  );
  const acquired = (row as Record<string, unknown>).acquired === true;

  if (!acquired) {
    console.log(`[risk-monitor] skipped: advisory lock ${LOCK_IDS.RISK_MONITOR} held`);
  ...
  try {
    ...
    console.log('[risk-monitor] executing stub');
