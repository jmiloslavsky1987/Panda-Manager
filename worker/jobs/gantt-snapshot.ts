...
export default async function ganttSnapshotJob(job: Job): Promise<{ status: string }> {
  // 1. Acquire transaction-scoped advisory lock (auto-releases at transaction end)
  const [row] = await db.execute(
    sql`SELECT pg_try_advisory_xact_lock(${LOCK_IDS.GANTT_SNAPSHOT}) AS acquired`
  );
  const acquired = (row as Record<string, unknown>).acquired === true;

  if (!acquired) {
    console.log(`[gantt-snapshot] skipped: advisory lock ${LOCK_IDS.GANTT_SNAPSHOT} held`);
    await db.insert(jobRuns).values({
      job_name: 'gantt-snapshot',
      status: 'skipped',
  ...
  try {
    ...
    console.log('[gantt-snapshot] executing stub');
