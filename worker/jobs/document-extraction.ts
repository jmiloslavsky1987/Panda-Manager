...
- milestone: { name, target_date, status, owner (verbatim name or null) } — STRICT SCOPE: Only extract major project gates: project kickoffs, team go-lives, production launches, phase completions, UAT sign-offs, and cutover events. Do NOT extract individual task completions, deliverable approvals, sub-milestones, or routine meeting dates.
...
- architecture vs arch_node vs integration:
  ...
  • arch_node = capability NODE within an "ADR Track", "AI Assistant Track", or "Incident Prevention Track" architecture diagram (e.g., "Alert Intelligence", "Incident Intelligence", "Knowledge Sources", "Change Risk Console") — use ONLY when document explicitly references these tracks
...
- task vs wbs_task:
  ...
  • wbs_task = item that belongs in a WBS hierarchy — must have a track (WBS template track — INFER from document context: "ADR" if BigPanda/enterprise deployment, "Biggy" if startup/SMB, "Incident Prevention" if document contains change-ticket / change-request / CHG- / RFC / CAB / change advisory board / risk score / change risk / ServiceNow change / JSM change / blast radius / freeze window / 5-category / weighted risk references. Default to "ADR" if unclear), a parent section (e.g. "Solution Design", "Platform Configuration"), and a level (1/2/3)
...
- arch_node track names: ONLY valid values are "ADR Track", "AI Assistant Track", and "Incident Prevention Track". If the document mentions a different track name, do NOT extract an arch_node entity — skip it entirely.
...
- task vs wbs_task: wbs_task = item that belongs in a WBS hierarchy with a track (WBS template track — INFER from document context: "ADR" if BigPanda/enterprise deployment, "Biggy" if startup/SMB, "Incident Prevention" if document contains change-ticket / change-request / CHG- / RFC / CAB / change advisory board / risk score / change risk / ServiceNow change / JSM change / blast radius / freeze window / 5-category / weighted risk references. Default to "ADR" if unclear), a parent section (e.g. "Solution Design", "Platform Configuration"), and a level (1/2/3). Rule: If document mentions WBS structure, ADR/Biggy tracks, or explicit template section names → wbs_task.
...
- arch_node track names (reference only): track names "ADR Track", "AI Assistant Track", and "Incident Prevention Track" were covered in pass 2.
...
TRACK CLASSIFICATION CUES (used downstream by passes 2–3 for arch_node / wbs_task track routing):
- ADR Track signals: alert correlation, alert ingestion, alert intelligence, incident intelligence, MTTR reduction, NOC, monitoring integrations, BigPanda standard delivery.
- AI Assistant (Biggy) Track signals: action plans, managed incident channels, AI assistant, runbook automation, conversational incident response.
- Incident Prevention Track signals: If the document mentions change tickets (CHG-, RFC), ServiceNow / JSM changes, change risk prediction, CAB / change advisory board, blast radius, CI criticality, freeze windows, or a 5-category weighted risk engine, classify as "Incident Prevention" track. Strong cues: "change ticket", "change request", "ServiceNow change", "JSM change", "risk score", "change risk", "CAB", "change advisory board". Supporting cues: "blast radius", "configuration item / CI", "freeze window", "blackout window", "5-category", "weighted risk", "risk engine", "approval workflow", "change approval".
...
function buildDedupeKey(item: ExtractionItem): string | null {
  ...
  // 40-char window matches the DB dedup prefix so cross-chunk paraphrases are caught
  const norm = (v: string | undefined | null) => (v ?? '').toLowerCase().trim().slice(0, 40);
