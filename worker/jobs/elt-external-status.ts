...
export default async function eltExternalStatusJob(job: Job): Promise<{ status: string }> {
  ...
  if (!runId) {
    console.error('[elt-external-status] runId required in job.data');
  ...
  try {
    ...
    await db.insert(outputs).values({
      ...
      skill_name: 'elt-external-status',
    ...
    console.error('[elt-external-status] failed:', message);
    return { status: 'failed' };
  }
}
