...
export default async function biggyWeeklyBriefingJob(job: Job): Promise<{ status: string }> {
  ...
  if (!runId) {
    console.error('[biggy-weekly-briefing] runId required in job.data');
  ...
  try {
    ...
    await orchestrator.run({
      skillName: 'biggy-weekly-briefing',
      projectId,
      runId,
      input,
    ...
    await db.insert(outputs).values({
      ...
      skill_name: 'biggy-weekly-briefing',
    ...
    console.error('[biggy-weekly-briefing] failed:', message);
    return { status: 'failed' };
  }
}
