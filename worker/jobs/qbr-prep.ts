...
export default async function qbrPrepJob(job: Job): Promise<{ status: string }> {
  ...
  if (!runId) {
    console.error('[qbr-prep] runId required in job.data');
  ...
  try {
    ...
    console.error('[qbr-prep] failed:', message);
    return { status: 'failed' };
  }
}
