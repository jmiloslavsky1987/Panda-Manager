...
**Current focus:** v12.0 — Architecture Sub-Capability Columns + Incident Prevention Track (Phase 87 CLOSED 2026-05-20; ready for milestone closure)

## Current Position

Phase: 87-incident-prevention-track-support (Incident Prevention Track Support — CLOSED 2026-05-20)
Plan: 9 of 9 complete (Plans 00..08 all done; Phase 87 CLOSED)
Status: Phase 87 CLOSED 2026-05-20 — all 16 phase-local requirements (IP-01..IP-16) verified end-to-end. 87-VERIFICATION.md PASSED 16/16. Plan 87-08 final human-verification approved by user: Task 1 (IP-01, IP-02) Docker DB SQL evidence — migration 0052 applied via direct psql ([85.2-01] precedent), 4 backfilled projects with {adr:true, biggy:true, incident_prevention:false}, arch_tracks=4, arch_nodes=68 (=4×17 IP nodes per project), idempotency UPDATE 0 on re-apply, backfill-protection guard verified. Task 2 (IP-15, IP-16) browser UAT across 3 test projects (regression / IP-only / mid-engagement flip) all approved. One inline UAT fix per [86-05] precedent: Phase 85 commit aa6ef069 had hardcoded all three WBS+Gantt subtab strip tabs without active_tracks gating; fixed in Panda-Manager d4bd889c (3 files, 23+/11-, well under 50 LoC budget). Briefing skill contract honored — zero changes to skills/briefing.md across the entire phase (track-agnostic by design per CONTEXT.md). v12.0 milestone ready for closure pending overall milestone wrap. Next: orchestrator runs phase-complete CLI; user can then run /gsd:complete-milestone for v12.0.
Last activity: 2026-05-20 — Phase 87 CLOSED. Plan 08 final human-verification PASSED: Task 1 (IP-01, IP-02) — migration 0052 applied via direct psql ([85.2-01] precedent), 4 backfilled projects with {adr:true, biggy:true, incident_prevention:false}, arch_tracks=4, arch_nodes=68 (=4×17 IP nodes per project), idempotency UPDATE 0 on re-apply, backfill-protection guard verified (manually set IP=true then re-applied → value preserved). Task 2 (IP-15, IP-16) — browser UAT approved across 3 test projects: Project A (regression, 2 tabs only) / Project B (IP-only wizard, 3 colored sections + Change Risk Console + 13 sub-caps + 1-column OnboardingDashboard + Team Gamma) / Project C (mid-engagement flip, IP arch+WBS+3-column dashboard+Team Gamma all appear on ON, hide on OFF preserving DB rows, no duplicate on re-ON). Inline UAT fix per [86-05] precedent: Phase 85 commit aa6ef069 had hardcoded all three WBS+Gantt subtab strip tabs without active_tracks gating; fixed in Panda-Manager d4bd889c (3 files, 23+/11-, under 50 LoC budget). 87-VERIFICATION.md written PASSED 16/16. Briefing skill contract honored — zero changes to skills/briefing.md across entire phase (track-agnostic by design per CONTEXT.md). All 16 IP-XX requirements closed. v12.0 milestone ready for closure.
...
- [87-05] Closed 3-key Zod shape inside `.optional()` pattern: `z.object({adr, biggy, incident_prevention}).optional()` — all three keys REQUIRED inside the object, outer `.optional()` allows omitting the field entirely. Forces the form to always send complete 3-key payloads (mitigates Pitfall 2 / JSONB merge silent-data-loss at request layer) while still allowing partial PATCH requests (e.g., name-only renames). Reusable for any future closed-shape JSONB-backed field.
- [87-05] Pre-transaction SELECT for retroactive-seed diff: `SELECT current.active_tracks` before opening db.transaction. Doubles as 404 check (return 404 if !current before transaction overhead). Diff computed in app-layer (`!currentTracks.incident_prevention && patch.active_tracks?.incident_prevention`), not SQL — keeps DB layer thin, makes the boolean readable for review.
- [87-05] UPDATE + conditional helper-call wrapped in single db.transaction: `await db.transaction(async (tx) => { tx.update + if (flippedOnIP) seedIncidentPreventionForProject(tx) })`. Atomic — partial helper failure rolls back the active_tracks flip too, preventing inconsistent "IP=true but no IP rows" state. Pattern reusable for any retroactive seeding on any track toggle.
- [87-05] `updated` row captured via outer `let` from inside `db.transaction` callback — Drizzle's `tx.update().returning()` returns within callback scope; assigning to outer `let` is canonical for surfacing the result post-commit. Future plans needing tx return values should follow this pattern.
- [87-05] ADR/Biggy retroactive false->true symmetry deliberately deferred — `seedADRForProject` and `seedBiggyForProject` helpers were never extracted (87-04 only extracted IP helper, single consumer). Route is structured for symmetric extension (add `flippedOnADR` boolean + helper call inside existing transaction); zero refactor cost when needed. Scope discipline > premature generalization.
- [87-05] Form-side always-complete-payload pattern: serialize ALL known keys of a JSONB field even when only one toggle changed. Single-line cost (`active_tracks: { adr: adrEnabled, biggy: biggyEnabled, incident_prevention: ipEnabled }`), eliminates whole class of silent-data-loss bugs from partial JSONB writes. Required complement to closed Zod shape on the server.
- [87-05] Order ADR -> Biggy -> Incident Prevention codified for checkbox row (per Phase 87 CONTEXT.md "Labels, ordering" rule). Stable across new-project wizard (87-04) and settings form (87-05). Future fourth track appends at the end (no reordering of existing labels).
- [87-05] No deviations on first execution — all grep verify gates passed, 4/4 IP-08+IP-09 tests GREEN, 5/5 projects.test.ts regression GREEN, TypeScript clean. Wave 0 RED test scaffold (87-00) gave precise gates; helper extraction (87-04) gave the seeding surface; Plan 87-05 was purely wire-up. Validates the GSD pattern of "Wave 0 tests + helper extraction" preceding the integration plan.
- [87-08] [86-05] precedent REAFFIRMED — late-discovered placeholder bugs from prior phases (Phase 85 `aa6ef069` here: WBS+Gantt subtab strip hardcoded all three tabs without `active_tracks` gating) can ship as inline UAT fixes inside the final verification plan, provided they meet the < 50 LoC + surface-area-only + no-test-regression criteria. Document in VERIFICATION.md + SUMMARY.md; do NOT open a gap-closure plan. This 87-08 case: 3 files (`app/customer/[id]/wbs/page.tsx`, `app/customer/[id]/wbs/WbsPageClient.tsx`, `app/customer/[id]/gantt/page.tsx`), 23 insertions, 11 deletions, Panda-Manager commit `d4bd889c`.
- [87-08] Three-project test matrix codified for any future track-add phase: (A) regression — existing project with the new track OFF (verify NO new UI surfaces); (B) new project created with ONLY the new track checked (verify isolated new-track UX); (C) mid-engagement flip — existing project that turns the new track ON then OFF then ON (verify retroactive seeding + render-layer hide preserving DB rows + idempotent re-seed). Each project exercises a distinct lifecycle path that the other two cannot.
- [87-08] Manual psql migration apply via direct `psql -d bigpanda_app -f /migrations/<file>.sql` is the canonical pattern for this Docker setup (`~/bin/panda-rebuild.sh` bypasses the `run-migrations.ts` service). Established at [85.2-01]; reaffirmed at [87-01] and [87-08]. Do NOT block phase closure waiting for run-migrations.ts to be fixed.
- [87-08] Backfill protection check (manually set `incident_prevention=true` on one project, re-apply migration 0052, verify the value is preserved by the `NOT (active_tracks ? 'incident_prevention')` guard) added beyond the plan's checklist — proves the [87-01] JSONB-additive idempotency guarantee in practice, not just in theory. Adopt as standard practice for any JSONB-additive migration.
- [87-08] Track ordering ADR → Biggy → Incident Prevention codified across EVERY UI surface (wizard, settings, OnboardingDashboard, Architecture, Integration Tracker, KpiStrip, OverviewMetrics, HealthDashboard, WBS, Gantt). Future fourth track simply appends at the end; NEVER reorder existing labels.
...
Last session: 2026-05-20T16:48:35Z
Stopped at: Completed 87-08-PLAN.md (Phase 87 final human-verification CLOSED — 87-VERIFICATION.md PASSED 16/16 with one inline UAT fix per [86-05] precedent)
Resume file: None
