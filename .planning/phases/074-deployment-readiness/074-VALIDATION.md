...
|----------|-------|
| **Framework** | vitest |
| **Config file** | bigpanda-app/vitest.config.ts (or none — Wave 0 installs) |
...
|---------|------|------|-------------|-----------|-------------------|-------------|--------|
| 74-01-01 | 01 | 0 | DEPLOY-01 | unit | `npm run test -- --run` | ❌ W0 | ⬜ pending |
| 74-01-02 | 01 | 1 | DEPLOY-01 | grep/static | `grep -r "localhost" bigpanda-app/app bigpanda-app/worker --include="*.ts" --include="*.tsx"` | ✅ | ⬜ pending |
| 74-01-03 | 01 | 1 | DEPLOY-01 | unit | `npm run test -- --run` | ❌ W0 | ⬜ pending |
| 74-02-01 | 02 | 1 | DEPLOY-01 | unit | `npm run test -- --run` | ❌ W0 | ⬜ pending |
...
*If existing vitest infrastructure exists, Wave 0 only adds test files.*
