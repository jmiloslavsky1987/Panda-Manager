// Global calendar import route — list Google Calendar events (GET) + import as draft time entries (POST)
// No projectId path param — all project selection is per-event via dropdown
    ...
    const inserted = await db
      ...
      .values(
        toImport.map((item) => ({
          ...
          user_id: session.user.id,          // Phase 32: set user_id for multi-user scoping
